...
export class AdmissionComponent implements AfterViewInit {
  ...
  async handleInitialSubmit(event: Event, formType: 'elementary' | 'junior' | 'senior' | 'higher'): Promise<void> {
    ...
    try {
      ...
      const { error } = await this.supabaseService.client.from('admissions_tb').insert(data);
      if (error) {
      ...
      this.container.nativeElement.innerHTML = `
        <div class="thank-you-message">
          ...
          <button id="login-button" type="button" class="bg-blue-600 text-white py-2 px-4 rounded hover:bg-blue-600 mt-4">Login</button>
        </div>
      `;
  ...
  showLoginForm(event?: Event) {
    ...
    this.container.nativeElement.innerHTML = `
      ...
      <button id="dashboard-go" type="submit" class="w-full bg-blue-600 text-white py-2 rounded hover:bg-blue-600">Login</button>
    `;
    ...
    const forms = {
      elementary: `
        <form id="elementary-form" class="space-y-4">
          ...
          <button type="submit" class="w-full bg-blue-600 text-white py-2 rounded hover:bg-blue-600">Submit</button>
        </form>
      `,
      junior: `
